...
class ListBook extends React.Component {
  ...
  render() {	  
    return(<div className="list-of-books">
        <div className="list-title-books">
          <h1>MyReads</h1>
        </div>
              ...
              <div className="bookshelf-books">
                <ol className="books-grid">
                  { this.props.allBooks.filter(book => book.shelf === 'currentlyReading')
                    .map(book => (
                      <Book key={book.id} book={book} onUpdateBook={(book, shelf) => this.updateBooks(book, shelf)}></Book>
                    ))}
                </ol>
              </div>
            </div>
              ...
              <div className="bookshelf-books">
                <ol className="books-grid">
                  { this.props.allBooks.filter(book => book.shelf === 'read')
                    .map(book => (
                      <Book key={book.id} book={book} onUpdateBook={(book, shelf) => this.updateBooks(book, shelf)}/>
                    ))}
                </ol>
              </div>
            </div>
